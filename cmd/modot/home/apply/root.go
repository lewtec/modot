...
import (
	...
	"github.com/lewtec/modot/internal/apply"
...
func Schedule(ctx context.Context, dryRun, showNoop bool) func() error {
	...
	taskgroup.Go(ctx, taskName, taskgroup.Control, func(ctx context.Context, s *taskgroup.Status) error {
		...
		stateStore, err := deployer.NewFileStateStore(filepath.Join(home, ".config", "modot", "state.json"), home)
		if err != nil {
			...
			&dotfiles.FuncHook{
				AfterFn: func(ctx context.Context, actions []deployer.Action, execErr error) error {
					...
					for _, action := range actions {
						...
						if home == liveHome && action.Desired.File != nil && deployer.GetTarget(action.Desired) == filepath.Join(home, ".config", "modot", "dconf.marker") {
