...
// "modot home config" and "modot codebase config".
...
Examples:
  modot %s config get workspaces.www
  modot %s config get desktop.wallpaper.dir
  modot %s config get desktop.wallpaper`, scope, scope, scope)
}
...
// Layers lists discovered modot.cue layers.
type Layers[M Mode] struct {
	Format cmd.EnumArg[cmdarg.LayersFormat] `short:"f" long:"format" help:"Output format (paths, table)" default:"paths"`
}

func (Layers[M]) Description() string { return "List discovered modot.cue layers" }
...
func (l *Layers[M]) Run(ctx context.Context) error {
	...
	if l.Format.Value() == cmdarg.LayersTable {
