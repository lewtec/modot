...
CUE config (`modot.cue`) drives everything.
...
- Registry install check: `mise run test:registry-install` (sets `MODOT_TEST_TOOL_INSTALL=1`) installs each lewkit catalog tool. Not part of `mise release`. Multi-target CI: `.github/workflows/registry-install.yml` (linux amd64 + arm64). Per-tool failures append to `GITHUB_STEP_SUMMARY` from `internal/tool/registry_install_test.go`.
...
- `cmd/modot/root.go`: `pkg/driver/prelude`; tool/check preludes load from the cmds that need them
...
Never import driver prelude except from `cmd/modot/root.go`. Tool/check preludes: from the cmd that needs them, not from `pkg/`. CLI groups export `Command` and embed generated `children`.
...
- Change how a tool locks: that tool's `Pin` method. Modot copies `Pin` onto the lock row.
...
For complex subsystems, mirror `skills/modot/references/templates.md`: decision tree, concrete examples, short reference tables. Usage skill material goes under `skills/modot/`.
...
- Import driver prelude only from `cmd/modot/root.go`.
- Only leaf taskgroup tasks take IO/CPU/Internet (`x/taskgroup` package doc).

Rest is in AGENTS.md and `skills/modot/`.
