...
Modot distinguishes artifact kinds. Choosing the wrong kind is the main
...
`.d.tmpl` fragments and CUE `file` unify as keyed `lines` slots.
...
- Root config: broader modot config (e.g. `.root.hosts`,
  `$.root.browser.webapp`)
...
{{- file (printf "modot-webapp-%s.desktop" $name) }}
[Desktop Entry]
Name={{ titleCase $name }}
...
4. Marker `<<<MODOT_FILE:..>>>` (from `file`/`endfile`): multi-file outputs
5. Normal file: symlink / static place
6. Compares with managed state (e.g. under `~/.local/share/modot/state.json`)
...
`modot home apply --help`. Loop: `plan-and-apply.md`. Do not maintain a
...
- Engine/code changed but outputs look frozen: warm module/tool caches; use
  root `--no-cache` / `MODOT_NO_CACHE` for a full cascade (`docs/specs/no-cache.md`).
