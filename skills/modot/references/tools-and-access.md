...
Essential: access modes + gotchas. Live details: `modot tool --help` and
`modot tool with --help` (spec formats, examples, resolution). Do not copy
help Examples into this file.

## Model

A tool is a versioned program obtained through a backend (registry/catalog,
`github:…`, `mise:…`, …). Modot can install into a store, expose shims, run
...
| `tool install` (and normal install flows) | On disk in tool store | Want it available via modot going forward |
| Shims (`…/modot/shims` on PATH) | Until shim/PATH removed | Daily `rg` / `uv` in an interactive shell |
...
Under `~/.local/share/modot/` conceptually: `tools/` (store), `shims/`
(PATH entries). Install without shims/PATH means "installed but my shell doesn't
see it."

## `tool with` (model only)

Full argv/spec grammar and examples: `modot tool with --help`.
...
— roles and flags from `modot tool --help` / subcommand help.
...
- Not apt/brew: modot store/shim model.
