...
Modot loads CUE configuration from different places depending on which
...
the package root of `modot.cue`. Authoring style is open; see `cue.md`.
...
| Context | Rough idea |
|---------|------------|
| Home / dotfiles | Config tied to the user environment and `$DOTFILES` / `~/.dotfiles`-style trees; home commands use this world. |
| Home directory | A `modot.cue` directly under the user home may participate in home loading. |
| XDG / config dir | Another possible home-config location depending on setup. |
| Codebase | Walk up from the current (or project) directory for the nearest `modot.cue`. |

Codebase / walk-up behavior matters a lot:

- Nested git repos generally do not inherit an outer repo's `modot.cue`
  when searching upward across that boundary. A project inside another project
  may have its own cue — or none.
- Closest cue wins in walk-up scenarios; parent cues are not merged across that
  search just because they exist higher up.

Know which tree `home` vs `codebase` is operating on, and which `modot.cue`
path that implies.
...
| Cue | Home/dotfiles/config-dir family | Nearest project `modot.cue` |

Same binary, different loaders and goals. See also `plan-and-apply.md`.

## Lockfile placement

`modot.lock.json` sits next to the workspace / config it belongs to (module
...
3. The actual `modot.cue` path that should apply (open it; don't assume
   `~/.dotfiles/modot.cue` if the user lives elsewhere).
4. Nearby `modot.lock.json` if modules/tools pins are involved.
...
- Valid CUE ignored by modot: wrong root key, wrong file, or a command
  family that never loads that path. Evaluation succeeding in abstract CUE tools
  does not mean modot loaded it.
...
- Home apply vs codebase apply: same word "apply", different targets. Home won't
  fix a project lint config in the repo, and codebase won't converge `~/.config`
  from dotfiles modules.
- Lock next to another workspace: refreshed lock in tree A, apply in tree B;
  pins and intent diverge.
