...
modules (bundles of files, templates, settings), resolves inputs (where module
sources come from), pins versions in a lockfile, and applies the desired tree
into home (dotfiles) or a codebase.
...
- Drivers: imperative OS actions (audio, brightness, WM, screenshot, open, …),
  selected by compatibility and weights in config.
...
```text
modot.cue  -->  modules + inputs (+ other config)
        |                    |
        |                    +--> modot.lock.json  (resolved pins)
        |                    |
...
Templates live inside modules (and related source layout). Apply materializes
them; editing a template alone only changes source.
...
Classify the task into one row before reaching for flags.
...
2. `init` — starter `modot.cue`, example module under dotfiles, host/IP hints.
...
- `modot.cue`: primary human/agent intent.
- `modot.lock.json`: resolved pins (sources, hashes, tool versions). Useful
  for reproducibility and automation (e.g. Renovate-oriented fields). Not where
  you design the environment.
- Module trees: e.g. `modules/<name>/` under dotfiles for `input: "self"`.

Which physical `modot.cue` loads depends on home vs codebase vs walk-up;
see `config-and-roots.md`.

## Related docs in the modot source repo

| Doc | Audience |
|-----|----------|
| `README.md` | One-line terms |
| `AGENTS.md` / `CODEMAP.md` | Contributing to modot itself |
| This skill (other `references/`) | Modules, tools, drivers, cue, plan/apply |

Templates are fully documented in `references/templates.md` (canonical).

This skill is for operating modot, including in other repos that only
consume it.
