...
Whole file is essential and short. Invocation/flags: `modot home plan|apply
...
(`modot --help`). `--no-cache` (or `MODOT_NO_CACHE`) cold-starts
...
| Area | Role |
|------|------|
| lint / format | Run CUE-defined checks (`lint` / `formatter`; may pull tools lazily). `lint --review` → GHA annotations on the relevant diff |
| config | Inspect codebase modot config |
