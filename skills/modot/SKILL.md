...
name: modot
description: >
  Mental models, subsystems, and gotchas for using modot (declarative
  environment / modules / templates / tools / drivers / plan-apply). Reference-
  centric: not a --help mirror, not for implementing modot internals.
  Triggers: modot, modot.cue, modules, templates, tool with, shims,
  home apply, codebase apply, mod lock/tidy, drivers, init, self-install.
---

# Modot (usage)

When debugging someone's setup, read their `modot.cue` and modules first.
...
|------------|-----------------|----------|
| Flags, arg shapes, invocation examples | `modot … --help` (and `Example`/`Long` on the command) | Skill body, tables of every subcommand |
...
| Modot host contracts for CUE (roots, evaluated keys, merge edges) | `references/cue.md` + `modules.md` | cue-language skill (it points here instead) |
| Hard contributor rules (CUE schema order, preludes) | `AGENTS.md` / `CODEMAP.md` | This skill (usage only) |
| User's real config | Their `modot.cue` + modules on disk | Assuming init shape is universal |
...
| Deep | Modot + CUE host contracts | `references/cue.md` | When writing/refactoring modot cue |
...
Paths are relative to `skills/modot/` unless noted. Usually one essential
ref is enough; add deep material only when editing that subsystem.

## With `cue-language` (sibling skill)

Both may trigger on `modot.cue`. Order matters:
...
3. Both when authoring non-trivial modot config: cue-language (legal CUE),
...
cue-language answers as a stand-in for which `modot.cue` loads or whether
to plan/apply.

## Not this skill

- Implementing modot internals: `AGENTS.md` / `CODEMAP.md`.
- Argv encyclopedias: `modot [path…] --help`.
...
- Non-trivial cue for modot: `skills/cue-language/` for mechanics, then
  `references/cue.md` + `modules.md` for host contracts
- CUE unify/`_|_`/defs only (not modot behavior): `skills/cue-language/`
