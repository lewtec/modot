...
Workspaced loads CUE configuration from different places depending on which
...
- Nested git repos generally do not inherit an outer repo's `workspaced.cue`
...
Know which tree `home` vs `codebase` is operating on, and which `workspaced.cue`
path that implies.
...
- Valid CUE ignored by workspaced: wrong root key, wrong file, or a command
  family that never loads that path. Evaluation succeeding in abstract CUE tools
  does not mean workspaced loaded it.
