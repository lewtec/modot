...
surprises). Load this file when the problem is workspaced's contract with
evaluated config.

Deep by default (only when authoring/refactoring workspaced cue). For "what is
workspaced" without editing cue, skip this file. Universals: SKILL.md.

## Stance

CUE is flexible here. Workspaced cares about the evaluated configuration:
...
packages — whatever still evaluates to what workspaced understands. Use
cue-language for how those constructs work; use this file for whether workspaced
will honor the result.

Prefer the user's existing `workspaced.cue` as the local dialect. Init's minimal
...
- Keep a list in CUE for your ergonomics, then project it to a keyed struct at
  the boundary workspaced merges (see merge note below).
...
- Schema is not a style guide: schema limits what workspaced decodes; it doesn't
  mean authoring must be minimal or non-DRY.
- Evaluates in `cue eval`, missing in apply: wrong root/key/command family, or
  field not part of workspaced's decoded model.
