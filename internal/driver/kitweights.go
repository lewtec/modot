...
var kitIface = map[string]string{
...
// kitID maps driver slugs that were renamed when the implementation moved.
var kitID = map[string]string{
...
// ApplyKitWeights copies CUE driver weights onto lewkit's registry.
// Slugs absent from the CUE map keep the factory Weight.
func ApplyKitWeights(cue map[string]map[string]int) error {
	overlay := map[string]map[string]int{}
	for iface, weights := range cue {
		name := iface
		if mapped, ok := kitIface[iface]; ok {
			name = mapped
		}
