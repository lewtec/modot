...
//   - MODOT_FORCE_DRIVER=<id>          (applies to any interface)
//   - MODOT_FORCE_RSYNC_DRIVER=<id>    (for rsync.Driver)
//   - MODOT_FORCE_CLIPBOARD_DRIVER=<id>
...
func forceDriverFromEnv(ifaceName string) string {
	if v := os.Getenv("MODOT_FORCE_DRIVER"); v != "" {
		return v
	}
	// Derive a friendly key, e.g.
	//   "github.com/lewtec/modot/internal/driver/rsync.Driver"        -> MODOT_FORCE_RSYNC_DRIVER
	//   "github.com/lewtec/modot/internal/driver/clipboard.Driver"    -> MODOT_FORCE_CLIPBOARD_DRIVER
	//   "github.com/lewtec/modot/internal/driver/dialog.Chooser"      -> MODOT_FORCE_DIALOG_CHOOSER_DRIVER
	key := ifaceName
	...
	key = "MODOT_FORCE_" + strings.ToUpper(key) + "_DRIVER"
...
func Get[T any](ctx context.Context) (T, error) {
	...
	// Apply any MODOT_FORCE_*_DRIVER override for testing/debug.
