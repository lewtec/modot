...
func switchToWorkspace(ctx context.Context, ws string, move bool) error {
	d, err := lewdriver.Get[kitwm.Driver](ctx)
	if err != nil {
		return err
	}
...
func NextWorkspace(ctx context.Context, move bool) error {
	...
	if runtimeDir == "" {
		runtimeDir = filepath.Join(os.TempDir(), fmt.Sprintf("modot-%d", os.Getuid()))
	}
	modotDir := filepath.Join(runtimeDir, "modot")
	if err := os.MkdirAll(modotDir, 0700); err != nil {
		logging.ReportError(ctx, err)
	}

	wsFile := filepath.Join(modotDir, "last_ws")
	lastWS := 10
